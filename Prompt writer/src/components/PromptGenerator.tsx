...
import { Prompt, PromptTemplate } from '../App';
import { apiService, generateFallbackPrompt, checkN8nAvailability } from '../services/api';
...
export function PromptGenerator({ onPromptGenerated, existingPrompts }: PromptGeneratorProps) {
  ...
  const generateAIPrompt = async () => {
    ...
    try {
      ...
      const isN8nAvailable = await checkN8nAvailability();
      
      if (isN8nAvailable) {
        // Use n8n backend for AI generation
        const response = await apiService.generateOptimizedPrompt({
          prompt: promptText,
        ...
        if (response.success && response.data) {
          const enhancedPrompt = `Optimized AI Prompt:\n\n${response.data.optimizedPrompt}\n\n💡 AI Suggestions:\n${response.data.suggestions.map((suggestion, index) => `${index + 1}. ${suggestion}`).join('\n')}\n\n🔧 Improvements Applied:\n${response.data.improvements.map((improvement, index) => `${index + 1}. ${improvement}`).join('\n')}`;
