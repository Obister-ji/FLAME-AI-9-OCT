...
const ClerkProvider: React.FC<ClerkProviderProps> = ({ children }) => {
  ...
  if (!publishableKey ||
    ...
    console.warn('Prompt Writer: Clerk publishable key not properly configured. Please add your actual Clerk publishable key to .env file.');
    return <>{children}</>;
  }
  
  // Log that Clerk is properly configured (for debugging)
  console.log('Prompt Writer: Clerk is properly configured with publishable key:', publishableKey.substring(0, 10) + '...');
