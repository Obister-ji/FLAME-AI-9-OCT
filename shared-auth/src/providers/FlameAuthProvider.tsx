...
interface FlameAuthProviderProps {
  children: React.ReactNode;
  appName?: string;
}

const FlameAuthProvider: React.FC<FlameAuthProviderProps> = ({ 
  children, 
  appName = 'Flame AI' 
}) => {
  ...
  if (!publishableKey ||
    ...
    console.warn(`${appName}: Clerk publishable key not properly configured. Please add your actual Clerk publishable key to .env file.`);
    return <>{children}</>;
  }
  
  // Log that Clerk is properly configured (for debugging)
  console.log(`${appName}: Clerk is properly configured with publishable key:`, publishableKey.substring(0, 10) + '...');
...
export default FlameAuthProvider;